...
def run_code(train_lists):
    for train_model in train_lists:          
        [subset, num_layers, hidden_dim, dropout, max_hidden, l1_penalty, l2_penalty, lr, model_selection, folder_idx, year] = train_model 
        ...
        train_idx_list = range(12 * year)
        valid_idx_list = range(12 * (year - 1), 12* year)
        if year < 38:
...
def get_tuned_network():
    temp_results =[[range(46, 60), 1, [2**6], 0.95, 6, 0.0, 0.001, 0.01, 'natural']] 
    result = []
    for temp_result in temp_results:
        for j in range(1,9):
            for k in range(1, 39):
                result.append(temp_result+[j]+[k])
    return result
