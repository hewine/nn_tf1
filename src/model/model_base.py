...
class ModelBase:
	...
	def _build_train_op(self, loss, scope, reg_l1 = 0, reg_l2 = 0, loss_factor=1.0):
		...
		elastic_regularizer = tf.contrib.layers.l1_l2_regularizer(
		scale_l1= reg_l1, scale_l2 = reg_l2,  scope=scope)
		regularization_penalty = tf.contrib.layers.apply_regularization(elastic_regularizer, trainable_variables)

		regularized_loss = loss + regularization_penalty # this loss needs to be minimized

		return tf.contrib.layers.optimize_loss(
			loss = regularized_loss * loss_factor,
